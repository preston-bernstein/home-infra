...
- **RAG Engine** — the service (LightRAG today, MiniRAG mid-migration) that indexes the Obsidian **Vault** and answers queries. NAS `:9621` (LightRAG), `:9623` for MiniRAG in repo compose (moved off `:9622` 2026-07-03 — that port is **occupied live by an undocumented `lightrag-trading` container, still unexplained — confirm with Preston before touching :9622** (see `home-infra-architecture-contract` drift register)).
- **vault-indexer** — nightly container on the NAS that POSTs vault files to the RAG Engine (cron `0 4 * * *` per `vault-indexer/crontab`, and spec prose now matches as of 2026-07-03).
