...
| `10.0.0.250:5000/minirag:latest` | cloned HKUDS/MiniRAG source | NAS (**deployed and health-verified 2026-07-03**, parallel to production LightRAG — not cut over) | **No pre-built GHCR image exists** — must build from source. **Traps:** the upstream Dockerfile `COPY`s a `.env` that is not in their repo (`touch .env` in the clone); `requirements.txt` omits `sentence-transformers` (crash-loops otherwise); a naive `pip install` pulls a ~5GB CUDA `torch` wheel unless pinned to CPU-only first. See `minirag-migration-campaign` Phase 2 for the exact fixes. |
