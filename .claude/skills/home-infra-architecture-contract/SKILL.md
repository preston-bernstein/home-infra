...
| 9 | `wiki-ingest.py` docstring: `--semantic-lint` runs "structural + semantic (LLM) lint" | A bare `--semantic-lint` ALSO runs a full ingest; safe lint-only: `--lint --semantic-lint` — see `home-infra-failure-archaeology` F10 | Medium (accidental ingest mutates the wiki and deletes captures) | One-line code fix via `home-infra-change-control`; until then use the safe invocation |
