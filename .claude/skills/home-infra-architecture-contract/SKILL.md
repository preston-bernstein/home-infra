...
| 8 | NAS compose declares `registry` (:5000), `minirag` (`9623:9721`, moved off `:9622` — Gate 0a), `watchtower` | **`minirag` DEPLOYED and health-verified live 2026-07-03** (PR #3 — see `home-infra-failure-archaeology` F12); `registry` intentionally not running (Route B); `watchtower` still not running; live NAS also runs `lightrag-trading` (0.0.0.0:9622→9621) which appears in no repo file | **Low — minirag live and healthy; only registry(intentional)/watchtower gaps remain** | `minirag-migration-campaign` Phase 4/5 for the remaining index+MCP-compat work; `lightrag-trading` ownership is a separate, still-open mystery; watchtower: start it or remove it from compose |
