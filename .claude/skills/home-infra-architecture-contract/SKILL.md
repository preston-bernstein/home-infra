...
## Load-bearing design decisions (distilled from ADRs 0001–0013 + CONTEXT.md)
...
8. **MiniRAG over LightRAG — the in-flight migration** (ADRs 0010, 0011, committed `ebc8e9e`; minirag deployed and health-verified live 2026-07-03, PR #3; still parallel to production LightRAG, not cut over). LightRAG's entity extraction needs structured JSON the 16GB-VRAM-fitting models can't reliably produce (~35% doc failure measured on llama3.1:8b; HKUDS recommends 32b+, which doesn't fit). MiniRAG (same lab) is SLM-first, but its confirmed API surface (`/documents/scan`, `/documents/upload`, `/documents/text`, `/documents/batch`) is a scan-based ingestion model, not LightRAG's submit-and-poll `track_id` pattern that both `vault-indexer/indexer.py` and lightrag-mcp are built around — reuse of either without adaptation is now a confirmed blocker, not just an unverified assumption (see `home-infra-failure-archaeology` F12). Embeddings switch mxbai-embed-large → bge-m3 at the same time (ADR 0011: mxbai degrades past ~1k tokens; bge-m3 handles 8192; free switch since full re-index anyway). Execution lives in `minirag-migration-campaign`.
...
| 2 | ~~`mcp/lightrag/README.md`: package `daniel-lightrag-mcp`, SSE, port 3001~~ | **CLOSED 2026-07-03** — README rewritten (`lightrag-mcp`, streamable-http, :3002, direct endpoint not nginx); Dockerfile's `EXPOSE`/CMD port corrected to 3002 to match compose (no behavior change — compose already overrode the CMD args) | — | Resolved |
| 3 | ~~`docs/specs/lightrag-vault-indexer.md` prose said "2am cron"~~ | **CLOSED 2026-07-03** — spec now says 4am, matching committed `vault-indexer/crontab` (`0 4 * * *`). Watchtower compose schedule `"0 0 4 * * *"` is still the same 4am hour (co-scheduling risk, unexamined — see `home-infra-failure-archaeology` F14) | — | Resolved (doc drift); the 4am/4am overlap itself is a separate, still-open scheduling question |
...
| 5 | ~~Spec `docs/specs/lightrag-vault-indexer.md`: state at `/volume1/docker/vault-indexer/`~~ | **CLOSED 2026-07-03** — spec now says `/volume1/docker/ai/vault-indexer/`, matching `compose/nas/docker-compose.yml`; also fixed the illustrative compose snippet's `LIGHTRAG_API_KEY=CHANGE_ME` to `${LIGHTRAG_API_KEY}` | — | Resolved |
| 6 | ~~`compose/desktop/docker-compose.yml` header: deploy path `/var/data/docker/docker-compose.yml`~~ | **CLOSED 2026-07-03** — header now says `/opt/docker/librechat-stack/docker-compose.yml`, matching the SSH-verified real path | — | Resolved |
| 7 | ~~ADR 0006: "Use SSE transport for all Home MCPs"~~ | **CLOSED 2026-07-03** — ADR 0013 documents the spec-driven rename to `streamable-http`; ADR 0006 itself is left unedited per house style (superseded, not rewritten) | — | Resolved |
| 8 | NAS compose declares `registry` (:5000), `minirag` (`9623:9721`, moved off `:9622` — Gate 0a), `watchtower` | **`minirag` DEPLOYED and health-verified live 2026-07-03** (PR #3 — see `home-infra-failure-archaeology` F12), **but `vault-indexer/indexer.py`'s document API calls match none of MiniRAG's confirmed endpoints** (new finding, same F12) — minirag is up but not yet indexable by the existing code; `registry` intentionally not running (Route B); `watchtower` still not running; live NAS also runs `lightrag-trading` (0.0.0.0:9622→9621) which appears in no repo file | **Medium — minirag is live and healthy, but Phase 4 (indexing) is now blocked on an API-shape mismatch, not just readiness** | `minirag-migration-campaign` Phase 4 flags the blocker and needs a design decision (route through `home-infra-change-control`); `lightrag-trading` ownership is a separate, still-open mystery; watchtower: start it or remove it from compose |
...
- **`lightrag-trading` on NAS :9622 — FLAG ONLY, never resolve unilaterally.** Live container, Up 5h as of 2026-07-02, mapped 9622→9621, present in zero repo files. minirag no longer wants that port (moved to `:9623` 2026-07-03 — Gate 0a), so this is no longer a migration blocker, but its ownership is still unexplained. It may be a deliberate second LightRAG instance for a trading corpus (name suggests so — ASSUMPTION). **Required action: ask Preston who owns it and where its config lives. Do not stop, rename, or re-port it.**
...
- ADR distillations from `docs/adr/0001..0013` (all committed); vocabulary from `CONTEXT.md`; sync contract and lightrag-trading interpretation are labeled ASSUMPTIONS.
- Re-verification one-liners (run all before trusting volatile rows):
  ...
  - Drift #2 (closed 2026-07-03): `grep -n 'daniel\|3001\|SSE' /Users/prestonbernstein/dev/home-infra/mcp/lightrag/README.md /Users/prestonbernstein/dev/home-infra/mcp/lightrag/Dockerfile` (expect no matches)
