...
| 8 | NAS compose declares `registry` (:5000), `minirag` (`9623:9721` as of 2026-07-03, moved off `:9622` — Gate 0a), `watchtower` | Live NAS (docker ps 2026-07-02) runs NONE of those three, and DOES run `lightrag-trading` (0.0.0.0:9622→9621, Up 5h) which appears in no repo file | **Medium — port conflict resolved; registry/minirag/watchtower deploy still pending** | `minirag-migration-campaign` handles the registry/minirag deploy; `lightrag-trading` ownership is a separate, still-open mystery (no longer a migration blocker); watchtower: start it or remove it from compose |
